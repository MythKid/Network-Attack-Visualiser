...
Alerts are low-volume and the deployment is single-node, so SQLite (in WAL mode for concurrent dashboard reads during writes) is sufficient and dependency-free. **Trade-off:** single-writer concurrency and no horizontal scale, accepted for a lab-scale tool; the repository layer isolates storage so a future engine swap is localised.
