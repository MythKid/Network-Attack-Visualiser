...
**Document status:** Partially implemented. The testing philosophy (§1), the deterministic clock-injection harness (§2) and the Phase 2 unit-test coverage (§3), together with the synthetic-event and detection-engine tests, are **implemented**. **Implemented in Phase 3:** the cooldown/deduplication tests (§4) and the integration and WebSocket tests (§6, §7) — including exact cooldown boundaries on the same injected-logical-time harness, transaction/rollback and thread-ownership storage tests, ingest limit/skew tests, and an automated **real-uvicorn** WebSocket test (real upgrade, real client socket) beyond the in-process TestClient, since the TestClient fakes the WebSocket transport. **One §6 item is deferred by design:** the tertiary backend event filter test moves to **Phase 6** with the Compose lab — the filter matches the lab's static addressing (`172.28.0.30:8000`), which does not exist before the lab does. **Implemented in Phase 5:** the malformed-packet corpus (§5, extended with VLAN and IPv4-fragment cases) and the replay end-to-end tests (§8), plus converter/limits/pacing/resource-cleanup unit tests, the acceleration-invariance check (alerts and statistics), the three repeated-replay characterisations, REST visibility of externally-committed replay rows, the absence of WebSocket deltas from a separate replay process (bounded-timeout negative assertion), and the CLI exit-code mapping. Everything else mapped to a later phase in §13 remains **planned**: frontend tests (§9), Docker verification (§10), AI-layer tests (§11) and the coverage gate (§12). See [DEVELOPMENT_PHASES.md](DEVELOPMENT_PHASES.md) and [PROJECT_PROGRESS.md](PROJECT_PROGRESS.md) for current status.
...
- Missing L4 headers.
- **802.1Q VLAN tags** (single and stacked) — traversed to the inner IPv4 layer.
- **IPv4 fragments** — non-first fragments carry no L4 header.

Expectation: no exceptions escape; each frame is either dropped or normalised to a valid `PacketEvent`. As implemented, the typed conversion result classifies each physical record as `emitted` / `unsupported` / `invalid` / `parse_error`: only **IPv4** frames are emitted (unknown L4 → `protocol = OTHER`; non-first fragments and missing-L4 frames keep null ports/flags); **IPv6, ARP and other non-IPv4 frames are safely dropped as `unsupported`** (no event), VLAN tags are traversed, and any dissection error drops only that frame. The parser never crashes the ingester.
