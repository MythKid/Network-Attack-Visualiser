...
**PCAP replay resource and safety controls (Phase 5).** Replay (`app/ingest/pcap_replay.py`, `scripts/replay_pcap.py`) runs **in-process and never crosses the network**, so it is exempt from the §5.1 HTTP-ingest limits; it enforces its own bounds instead. It streams with Scapy `PcapReader` (never `rdpcap`) under configurable caps — `REPLAY_MAX_FILE_BYTES` (checked via `stat` **before** the file is opened), `REPLAY_MAX_PACKETS` (physical records, incl. dropped frames), `REPLAY_BATCH_SIZE` and `REPLAY_MAX_SLEEP_S` — so memory and work stay bounded and no per-run alert-delta list is retained. The capture path is resolved and required to be an existing **regular file** before reading. Only packet **metadata** is extracted (the 5-tuple, TCP flags, wire length, timestamp); payloads are never read, stored or logged, upholding §7. There is **no replay HTTP endpoint and no upload mechanism** — the ingest token and body/batch limits remain live-path only. Provenance is forced to `replay`; it cannot be overridden.

**Licence posture (Phase 5).** Phase 5 adds Scapy (GPL-2.0-only) as a runtime dependency and imports it as a library; the project is relicensed **MIT → GPL-2.0-only** going forward as a conservative compliance posture. This is a project licensing decision, **not legal advice**; previously released versions retain their MIT licence and future relicensing depends on the agreement of the relevant copyright holders.
