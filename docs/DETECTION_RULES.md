...
**Document status:** Implemented. **Phase 2** delivered the detector interface contract (§2) with the source-aware `expire(source_type, now)`; the event-time, out-of-order and too-late policy (§2.1); the `portscan` (§3) and `synflood` (§4) detectors; the overlap policy (§6, no cross-detector deduplication); the scenario mapping (§7); and the configuration variables (§8). **Phase 3** delivered the Alert Engine and its cooldown/deduplication gate (§5): `CandidateAlert` objects become persisted `Alert` rows, the `*_COOLDOWN_S` values are consumed, duplicate triggers within cooldown reinforce the existing row (`alert.updated`) while triggers after it create a new row (`alert.created`), and updates are broadcast over the WebSocket feed ([API.md](API.md) §5). The gate's in-memory state is partitioned by `(source_type, detector_id)`, mirroring the detector-state isolation in §6. See [DEVELOPMENT_PHASES.md](DEVELOPMENT_PHASES.md) and [PROJECT_PROGRESS.md](PROJECT_PROGRESS.md) for current status.
